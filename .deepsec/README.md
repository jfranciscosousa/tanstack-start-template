...
Currently configured project: `tanstack-start-template` (target: `..`).
...
original init command. Checkpoints in `data/tanstack-start-template/setup/setup-state.json`
...
`--project-id tanstack-start-template` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/tanstack-start-template/`.
