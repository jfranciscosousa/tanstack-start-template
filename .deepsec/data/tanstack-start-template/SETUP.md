# Agent setup for `tanstack-start-template`

This is a deepsec scanning workspace. Project `tanstack-start-template` was just registered
(target: `..`). Setup is incomplete — `data/tanstack-start-template/INFO.md`
...
2. **Fill in `data/tanstack-start-template/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id tanstack-start-template
pnpm deepsec process --project-id tanstack-start-template
```
